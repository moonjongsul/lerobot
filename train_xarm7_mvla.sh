...
TRAIN_NAME="mvla_xarm7_eef_velocity_s${STAGE}_split"
DATE='260923'
...
VAL_EPISODES="$(python -c "import json;print('['+','.join(map(str,json.load(open('${SPLIT_FILE}'))['val']))+']')")"
# 헤드 오프라인 평가 주기. 체크포인트 주기의 배수로 둬서 평가가 나쁜
# 체크포인트가 어느 것인지 바로 짚이게 한다.
EVAL_FREQ=4000
